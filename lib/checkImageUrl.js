...
function cleanImageUrl(imgUrl) {
  try {
    const newImgUrl = (new url.URL(imgUrl)).toString();
    if (newImgUrl !== imgUrl) {
      console.warn("BestImage.cleanImageUrl: mismatched URL", imgUrl, newImgUrl);
    }
    return newImgUrl;
  } catch (err) {
    console.error("BestImage.cleanImageUrl: invalid", imgUrl, err);
    return "";
  }
}
...
function getImageSize(imgUrl, cb) {
  ...
  try {
    const newImgUrl = (new url.URL(imgUrl)).toString();
    if (newImgUrl !== imgUrl) {
      console.warn("BestImage.geImageSize:")
    }
  } catch (err) {
